...
def f(x,u):
    ...
    vy_dot = sigma * cs.cos(theta) * cs.sin(phi) - 2*omega*vx + 3*py*omega*omega
    vz_dot = sigma * cs.sin(theta) - pz*omega*omega
    w = sigma
...
opti.subject_to(X[6,-1]>=0) # Fuel consumption must be positive
opti.subject_to(T<=1813) 
# opti.subject_to(U[:,0]==0)
opti.set_initial(U[1:3,:], 0)
# ---- solve NLP              ------
opts = {};
opts["ipopt"] = dict(max_iter=5000)
...
plt.savefig("figures/P2_position_with_sigma_max_{}.png".format(sigma_max), dpi=300)
...
plt.minorticks_on()
plt.grid(visible=True, which="both", linestyle=":")
plt.savefig("figures/P2_velocity_with_sigma_max_{}.png".format(sigma_max), dpi=300)
...
plt.minorticks_on()
plt.grid(visible=True, which="both", linestyle=":")
plt.savefig("figures/P2_thrust_acceleration_with_sigma_max_{}.png".format(sigma_max), dpi=300)


plt.figure(4)
plt.clf()
plt.plot(sol.value(X[6,:]))
plt.title("")
plt.show()